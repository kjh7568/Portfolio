...
## 페이지 매핑
- 01 cover / 02 overview / 03~06 F1 / 07~10 F2 / 11~14 F3 / 15 TS-01 / 16 TS-02 / 17 retro
...
### meta
- Genre: `FPS 좀비 서바이벌 + 로그라이크 강화`
- Period: `2025.06.07 ~ 2025.06.27` / `(약 21일)`
- Team: `2인`
- Stack tags: `Unity` `URP` `C#` `NavMesh` `StateMachine` `ScriptableObject` `Coroutine` `TextMeshPro` `DOTween` `Post Processing`

### author
- 김지훈 / Jeehoon Kim / Unity Client Developer / rwrwg159@gmail.com / 010-0000-0000
...
### TOC (6항목)
1. `01` 프로젝트 개요 · 게임 흐름 — p.02
2. `02` 다중 무기 선택/전환 시스템 — p.03
3. `03` 상점 + 무기 강화 연동 — p.07
4. `04` 보스 AI · State 패턴 — p.11
5. `05` 트러블슈팅 · 시점 / NavMesh — p.15
6. `06` 회고 · 다음 개선 — p.17
...
### steps (4단계)
1. **01 입장 — 무기 선택**
   `로비에서 시작 무기를 고르고 스테이지에 입장합니다. 4개 슬롯(주무기·보조·근접·수류탄) 중 본인의 플레이 스타일에 맞는 무기를 결정합니다.`
2. **02 전투 — 좀비 웨이브**
   `웨이브마다 30~50마리의 좀비가 NavMesh 위를 추격해 옵니다. 무기 전환 키(1·2·F·G) 로 상황에 맞춰 대응하며 코인을 모읍니다.`
3. **03 상점 — 무기 강화**
   `웨이브 클리어 후 상점에서 코인으로 새 무기를 구매하거나 보유 무기를 레벨업·등급업합니다. Common → Rare → Epic → Legendary 4단계로 성장합니다.`
4. **04 보스전 — 패턴 대응**
   `최종 스테이지에서 보스 Nasty 가 등장합니다. 거리에 따라 일반 공격·스매시·원거리 Brass 패턴을 분기하므로, 거리 조절과 회피 타이밍이 핵심입니다.`

### features (3장)
1. **01 다중 무기 선택/전환** — `4슬롯 enum 과 추상 WeaponController 로 7종 무기를 단일 흐름에서 통합 관리합니다.` → p.03
2. **02 상점 + 무기 강화 연동** — `단일 StorePanelManager 로 아이템샵·무기/방어구 강화 3탭을 일원화하고 옵션 누적 연동을 보장합니다.` → p.07
3. **03 보스 AI · State 패턴** — `IBossState 인터페이스로 6개 상태를 객체 단위 분리해 거리·체력에 따라 자동 전환합니다.` → p.11

---

## PAGE 03 — F1 Cover · 다중 무기 선택/전환
...
- **2줄 타이틀:**
  ...
  무기를 골라 싸운다.
  ```
- **oneLiner_html:** `4개 슬롯 <span class="hl-blue">enum</span> 과 추상 <span class="hl-blue">WeaponController</span> 로 7종 무기의 입력·자세·애니메이션을 단일 흐름으로 통합한 시스템입니다.`

### why
- **label:** `구현 배경` / `Why this feature?`
- **body[0] (문제 인식):**
...
### pagesHint
- `p.04 · STRUCTURE` — `WeaponManager + WeaponController 추상 + 7개 구현체`
- `p.05·06 · DETAIL` — `슬롯 enum 으로 정형화 / 단일 입력 흐름 코드`
...
- **callout — 슬롯 정형화**
  `4슬롯 <code class="inline">WeaponSlot</code> enum 이 입력·자세·애니메이션 변경의 단일 분기점이 되어, 무기 추가가 매니저 코드를 거의 건드리지 않습니다.`
...
- **paragraphs_html:**
  1. `7종 무기를 21일 안에 모두 안정화해야 했고, 무기 추가가 빈번한 프로젝트 특성상 추가 비용을 줄이는 게 우선이었습니다.`
  2. `<code class="inline">WeaponSlot</code> enum 으로 슬롯을 정형화하고, <code class="inline">WeaponController</code> 추상 클래스로 Fire/Reload 만 무기별로 다르게 두었습니다. 데이터는 <code class="inline">WeaponDataSO</code> 로 외화해 디자이너가 인스펙터에서 등급·레벨 스탯을 조정할 수 있게 했습니다.`
  3. `이 구조로 무기 추가 시 SO 1개 + 구현체 1개만 늘리면 매니저 흐름에 자동 합류하며, 입력·자세·애니메이션 분기 코드 수정이 발생하지 않았습니다.`

---

## PAGE 06 — F1 Detail 2 (codeTabs)
...
- **tab 1 — `data` / `WeaponDataSO`**
  - lang: `CSHARP`
  - 발췌 (등급 enum + 레벨 테이블 + 등급별 레벨 범위):
    ```csharp
    public enum WeaponType { Akm, M4, Sniper, Shotgun, Ump, Pistol, Knife, Grenade }
...
- **tab 2 — `controller` / `WeaponController`**
  - lang: `CSHARP`
  - 발췌 (추상 + 공통 데미지/크리 계산):
    ```csharp
...
### resultCard
- **num:** `4` / **badge:** `RESULT`
- **body_html:** `4슬롯 enum + <code class="inline">WeaponController</code> 추상화 + <code class="inline">WeaponDataSO</code> 분리로, 7종 무기 추가 시 데이터·구현체 한 쌍만 늘리면 매니저 흐름에 자동 합류합니다.`

---

## PAGE 07 — F2 Cover · 상점 + 무기 강화 연동
...
- **2줄 타이틀:**
  ...
  더 강해진다.
  ```
- **oneLiner_html:** `단일 <span class="hl-blue">StorePanelManager</span> 로 아이템샵·무기 강화·방어구 강화 3탭을 일원화하고, 무기 인스턴스(<span class="hl-blue">Weapon</span>) 를 매개로 강화 시스템과 옵션 누적을 연동합니다.`
...
- **body[1] (해결 방향):**
  `해결 방향 : <b>StorePanelManager 단일 진입점에서 3탭 패널 활성/비활성과 입력 잠금을 통제하고</b>, Weapon 인스턴스를 매개로 강화 시스템(`UpgradeWeaponSystem`) 과 매니저(`WeaponManager.ApplyWeaponOption`) 가 옵션 누적을 동기화하도록 묶었습니다.`

### pagesHint
- `p.08 · STRUCTURE` — `StorePanelManager + 3탭 매니저 + Weapon 인스턴스 매개`
- `p.09·10 · DETAIL` — `상점 골격 단일화 / 강화 ↔ 옵션 적용 연동 코드`
...
- **callout — 매개체**
  `<code class="inline">Weapon</code> POCO 인스턴스가 상점·강화·매니저 사이 데이터 통로 역할을 하므로, 강화 결과가 실시간 무기에 즉시 반영됩니다.`
...
- **paragraphs_html:**
  1. `세 종류 패널을 21일 안에 회귀 없이 안정화하는 게 우선이었고, 입력 잠금·커서 상태가 한 곳에서 통제되지 않으면 보스전 직전에 마우스가 풀리는 치명적 버그로 이어집니다.`
  2. `<code class="inline">StorePanelManager</code> 가 패널 활성/비활성·커서·플레이어 입력 잠금만 책임지고, 실제 도메인 동작(랜덤 무기 생성·강화 비용·옵션 누적) 은 <code class="inline">ItemShopManager</code> / <code class="inline">UpgradeWeaponSystem</code> 등 도메인 매니저로 위임했습니다.`
  3. `이 구조로 패널 추가 시 <code class="inline">SwitchToXxx()</code> 메서드 한 개만 늘리면 되고, 입력 잠금 코드는 <code class="inline">OpenPanel/ClosePanel</code> 한 쌍에만 존재합니다.`

---

## PAGE 10 — F2 Detail 2 (codeTabs)
...
- **tab 2 — `shop` / `ItemShopManager`**
  - 발췌 (구매 시 이전 무기 드롭 + 옵션 적용):
    ```csharp
...
- **tab 3 — `upgrade` / `UpgradeWeaponSystem`**
  - 발췌 (등급업 시 옵션 한 줄 누적):
    ```csharp
...
### resultCard
- **num:** `4` / **badge:** `RESULT`
- **body_html:** <code class="inline">Weapon</code> 인스턴스를 매개로 상점·강화·매니저가 동기화돼, 등급업 직후 발사부터 누적된 옵션이 데미지·치명타 계산에 즉시 반영됩니다.`

---

## PAGE 11 — F3 Cover · 보스 AI · State 패턴
...
- **2줄 타이틀:**
  ...
  이긴다.
  ```
- **oneLiner_html:** `<span class="hl-blue">IBossState</span> 인터페이스로 6개 상태(Move/Attack/Smash/Stunned/Brass/Die) 를 객체 단위로 분리하고, <span class="hl-blue">거리·체력</span> 기반으로 상태를 자동 전환하는 보스 AI 입니다.`
...
### pagesHint
- `p.12 · STRUCTURE` — `IBossState + BossController + 6개 State 구현체`
- `p.13·14 · DETAIL` — `거대 switch 대신 객체 분리 / 거리 기반 자동 분기 코드`
...
- **callout — 거리 기반 자동 분기**
  `BossMoveState.UpdateState() 가 매 프레임 거리(<10f / <20f / 그 외)를 보고 Attack·Smash·Brass·추격으로 분기하므로, 보스가 "패턴을 가진 상대" 처럼 느껴집니다.`
...
- **paragraphs_html:**
  1. `보스 패턴이 5~6개 이상이고 후속 패턴 추가 가능성이 있었으므로, 회귀 위험을 줄이는 게 시간을 절약하는 길이었습니다.`
  2. `<code class="inline">IBossState</code> 인터페이스로 EnterState/UpdateState/ExitState 라이프사이클을 강제하고, <code class="inline">BossController.SwitchState()</code> 가 진입·종료 훅을 자동 호출하도록 했습니다. 시간 기반 복귀는 <code class="inline">StartDelay()</code> 코루틴 헬퍼로 일원화했습니다.`
  3. `이 구조로 신규 패턴(예: <code class="inline">BossBrassState</code>) 추가가 파일 1개 + <code class="inline">BossMoveState</code> 분기 한 줄 변경으로 끝났고, 기존 패턴은 회귀 없이 유지됐습니다.`

---

## PAGE 14 — F3 Detail 2 (codeTabs)
...
- **tab 1 — `interface` / `IBossState`**
  - 발췌 (인터페이스 + Controller 진입점):
    ```csharp
...
- **tab 3 — `attack` / `BossAttackState + BossSmashState`**
  - 발췌 (애니메이션 길이 후 자동 복귀):
    ```csharp
...
### resultCard
- **num:** `4` / **badge:** `RESULT`
- **body_html:** `<code class="inline">IBossState</code> 객체 분리 + <code class="inline">SwitchState()</code> 단일 진입점 + <code class="inline">StartDelay()</code> 헬퍼로, 패턴 추가가 한 파일 + Move 분기 한 줄 변경으로 끝나고 기존 상태에 회귀가 발생하지 않습니다.`

---

## PAGE 15 — TS-01 시점 변경 시 캐릭터 모델 노출
...
- **03 문제 해결 — `accent-bar`**
  ...
  - **inlineCode (발생 시점의 RotatePlayer):**
    - file: `PlayerController.cs`
    - lang: `CSHARP`
    - lines: `134\n135\n136\n137\n138\n139\n140\n141\n142\n143\n144\n145`
    - highlighted_html (발췌, 토큰 클래스 부여):
      ```
...
- **04 결과 — `good`**
  ...
  - result_html: `1인칭 시점에서 자신의 캐릭터 모델이 화면에 들어오는 문제가 사라졌고, 추가 1인칭 전용 팔 모델 제작이나 레이어 분리 작업 없이 캐릭터 본 한 곳의 회전 동기화만으로 몰입감을 회복했습니다.`

---

## PAGE 16 — TS-02 보스 공격 진입 시 NavMesh 추격 정지 누락
...
- **03 문제 해결 — `accent-bar`**
  `<b>(a)</b> <code class="inline">BossMoveState.UpdateState()</code> 의 거리 분기에서 근접 진입(<10f) 직전에 <code class="inline">agent.isStopped = true</code> 와 <code class="inline">transform.LookAt(target)</code> 을 호출하도록 변경. <b>(b)</b> 추격이 필요한 분기(원거리·중거리 추격) 에서는 <code class="inline">agent.isStopped = false</code> 로 명시적으로 풀어주는 패턴을 강제. <b>(c)</b> 결과적으로 진입/이탈 훅이 NavMesh 상태를 책임지게 돼, State 추가 시에도 정지/추격 일관성이 유지됩니다.`
  - **inlineCode:**
...
- **04 결과 — `good`**
  ...
  - result_html: `근접 패턴 진입 시 NavMesh 가 정지되고 보스가 플레이어를 정면으로 응시한 채 공격 모션을 재생하므로, 공격 판정과 위협감 모두 안정화됐습니다. 후속 패턴 추가 시에도 거리 분기 한 줄만 추가하면 같은 보장이 유지됩니다.`
...
### keep (2건)
1. **`IBossState 인터페이스 + State 객체 분리.`**
   `보스 패턴을 한 파일 = 한 상태로 격리한 덕분에, Brass 같은 후속 패턴 추가가 파일 1개와 분기 한 줄만 건드리면 끝났습니다. 진입/이탈 훅이 강제돼 애니메이터 트리거·플래그 누락도 사전 차단됐습니다.`
2. **`StorePanelManager 단일 진입점.`**
   `패널 활성/커서/입력 잠금을 한 곳에서 통제한 결과, 21일 동안 회귀 버그(패널 동시 활성·커서 풀림) 가 거의 발생하지 않았습니다. 패널 추가도 SwitchToXxx() 한 메서드만 늘리면 끝이었습니다.`

### problem (2건)
1. **`Find / Singleton 다중 사용으로 의존성이 암묵적이다.`**
   `<code class="inline">FindObjectOfType</code>, <code class="inline">Player.localPlayer</code>, 매니저 <code class="inline">.instance</code> 가 곳곳에 흩어져 있어, 어떤 클래스가 어떤 매니저에 의존하는지 코드를 따라가야만 알 수 있습니다. 매니저 초기화 순서 변경 시 NRE가 산발적으로 터졌습니다.`
2. **`옵션 적용/제거 시그니처 비대칭.`**
   `<code class="inline">WeaponManager.ApplyWeaponOption</code> 이 Weapon 전체용·단일 옵션용으로 오버로드되면서, Apply 와 Remove 의 호출 짝이 맞지 않으면 옵션이 누적되거나 사라지는 위험이 남았습니다. 단일 헬퍼(<code class="inline">ApplySingleOption</code>) 로 일부 정리했지만 완전히 통합하진 못했습니다.`

### try (3건)
1. **`이벤트 버스로 매니저 의존성 줄이기.`**
   `<code class="inline">CombatSystem.AddInGameEvent</code> 에서 검증된 이벤트 큐 패턴을 무기·상점·보스 사이 통신에도 확장해, 직접 인스턴스 참조 대신 이벤트로 느슨한 결합을 만들고 싶습니다.`
2. **`등급별 레벨 범위를 SO 자체에 두기.`**
   `<code class="inline">WeaponDataSO.gradeLevelLimits</code> 를 코드 상수 Dictionary 로 둔 것이 디자이너 손에 닿지 않는 결정점이었습니다. SO 인스펙터에 노출해 데이터 단계에서 밸런스를 조정하게 하고 싶습니다.`
3. **`State 객체 캐싱 / 풀링.`**
   `현재는 SwitchState() 마다 <code class="inline">new BossMoveState()</code> 식으로 객체를 새로 만드는데, 패턴이 잦은 보스라 GC 부담이 누적될 수 있습니다. State 인스턴스를 BossController 가 미리 보유하고 재사용하는 패턴으로 옮기고 싶습니다.`

---

## 추후 제작 예정 자산

- assets/techdocs/augment-zero/feature-1-uml.png
- assets/techdocs/augment-zero/feature-1-detail-1.png
- assets/techdocs/augment-zero/feature-1-detail-2.gif
- assets/techdocs/augment-zero/feature-2-uml.png
- assets/techdocs/augment-zero/feature-2-detail-1.png
- assets/techdocs/augment-zero/feature-2-detail-2.gif
- assets/techdocs/augment-zero/feature-3-uml.png
- assets/techdocs/augment-zero/feature-3-detail-1.png
- assets/techdocs/augment-zero/feature-3-detail-2.gif
- assets/techdocs/augment-zero/ts-01.png  (사용자 보유 슬라이드 활용 가능)
- assets/techdocs/augment-zero/ts-02.png
- Cover Hero (heroPlaceholder 사용 중)
- Overview Gameplay (gameplayPlaceholder 사용 중)
